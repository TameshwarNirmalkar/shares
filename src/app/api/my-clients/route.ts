...
import MyClientsModel from "@db/models/myclients";
...
export async function GET(request: NextRequest) {
  ...
  if (geth && hasBearer[0] !== "Bearer") {
    ...
    try {
      const { payload } = await jose.jwtVerify(`${hasBearer[1]}`, secret);
      await connectMongoDB();
      const response = await MyClientsModel.find({ uuid: payload.id });
      return NextResponse.json({ myClientList: response, success: true }, { status: 200 });
    } catch (error: any) {
      return NextResponse.json({ message: "Unauthorized", code: error.code, success: false }, { status: 403 });
    }
  }
...
export async function PATCH(request: NextRequest) {
  ...
  if (geth && hasBearer[0] !== "Bearer") {
    ...
    try {
      ...
      await connectMongoDB();
      await MyClientsModel.updateOne({ _id: payload._id }, { $set: { ...payload, lastUpdate: "$$NOW" } }, { upsert: true });
...
export async function DELETE(request: NextRequest) {
  ...
  if (geth && hasBearer[0] !== "Bearer") {
    ...
    try {
      await jose.jwtVerify(`${hasBearer[1]}`, secret);
      await connectMongoDB();
      await MyClientsModel.deleteOne({ _id: payload._id });
